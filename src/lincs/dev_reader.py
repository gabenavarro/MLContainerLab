...
@njit(fastmath=True)
def _compute_statistics(data: np.ndarray) -> Tuple:
    """
    Helper function to compute means and stds for a 2D array.
    
    Parameters
    ----------
    data : np.ndarray, shape (n_replicates, n_genes)
        Data matrix
    
    Returns
    -------
    means : np.ndarray, shape (n_genes,)
        Mean across replicates for each gene
    stds : np.ndarray, shape (n_genes,)
        Standard deviation across replicates for each gene
    """
    n_reps, n_genes = data.shape
    means = np.zeros(n_genes, dtype=np.float32)
    stds = np.zeros(n_genes, dtype=np.float32)
    denom = n_reps - 1 if n_reps > 1 else 1
...
def _compute_bai_with_credibility(
        sample_replicates: np.ndarray,      # shape: (n_sample_reps, n_genes)
        reference_signature: np.ndarray,    # shape: (m_ref_reps, n_genes)
        agreement_scale: float = DEFAULT_AGREEMENT_SCALE,
        prior_strength: float = 1.0
    ) -> tuple:
    """
    OPTIMAL VERSION: Pre-computes statistics, then parallelizes agreement calculation.
    
    This version is more efficient because it:
    1. Separates statistics computation (sequential) from agreement calc (parallel)
    2. Maximizes parallelization opportunities
    3. Reduces nested loop complexity
    ...
    reference_signature : np.ndarray, shape (m_ref_reps, n_genes)
        Reference gene expression signature with replicates
    agreement_scale : float
        Controls sensitivity of agreement probability
    prior_strength : float
        Beta prior pseudocount
    
    Returns
    -------
    bai_score, lower_credible, upper_credible, mean_agreement : tuple
    """
    n_sample_reps, n_genes = sample_replicates.shape
    m_ref_reps = reference_signature.shape[0]
    
    if n_sample_reps == 0 or m_ref_reps == 0 or n_genes == 0:
    ...
    sample_means, sample_stds = _compute_statistics(sample_replicates)
    reference_means, reference_stds = _compute_statistics(reference_signature)
    
    # Compute agreement probabilities (fully parallelizable)
    ...
    for j in prange(n_genes):
        ...
        pooled_std = np.sqrt(sample_stds[j]**2 + reference_stds[j]**2 + EPSILON_VARIANCE)
    ...
    if denom_post < EPSILON_DIVISION:
        denom_post = EPSILON_DIVISION
    ...
    z_critical = 1.96
    lower_credible = max(0.0, bai_score - z_critical * post_std)
    upper_credible = min(1.0, bai_score + z_critical * post_std)
    mean_agreement = np.mean(agreement_probs)
    
    return (bai_score, lower_credible, upper_credible, mean_agreement)


def compute_bai_with_exact_credible_intervals(
        sample_replicates: np.ndarray,      # shape: (n_sample_reps, n_genes)
        reference_signature: np.ndarray,    # shape: (m_ref_reps, n_genes)
        agreement_scale: float = DEFAULT_AGREEMENT_SCALE,
        prior_strength: float = 1.0,
        credible_level: float = CREDIBLE_INTERVAL_COVERAGE
    ) -> Tuple[float, float, float, float]:
    """
    Compute BAI with EXACT Beta credible intervals using scipy.
    
    This is the statistically correct version that should be used when
    numba performance is not critical. Uses proper Beta quantiles.
    
    CORRECTED: Properly handles 2D reference_signature.
    ...
    -------
    bai_score : float
        Bayesian Agreement Index (posterior mean)
    ...
    """
    # Validate inputs
    if reference_signature.ndim != 2:
        raise ValueError(f"reference_signature must be 2D, got shape {reference_signature.shape}")
    if sample_replicates.ndim != 2:
        raise ValueError(f"sample_replicates must be 2D, got shape {sample_replicates.shape}")
    if sample_replicates.shape[1] != reference_signature.shape[1]:
        raise ValueError(
            f"Gene dimension mismatch: sample has {sample_replicates.shape[1]} genes, "
            f"reference has {reference_signature.shape[1]} genes"
        )
    
    # Check for NaN/Inf
    if np.any(np.isnan(sample_replicates)) or np.any(np.isnan(reference_signature)):
        raise ValueError("Input arrays contain NaN values")
    if np.any(np.isinf(sample_replicates)) or np.any(np.isinf(reference_signature)):
    ...
    # Compute statistics for BOTH sample and reference
    sample_means = np.mean(sample_replicates, axis=0)
    sample_stds = np.std(sample_replicates, axis=0, ddof=1)
    
    reference_means = np.mean(reference_signature, axis=0)
    reference_stds = np.std(reference_signature, axis=0, ddof=1)
    
    # Compute z-scores and agreement probabilities
    diff = sample_means - reference_means
    pooled_std = np.sqrt(sample_stds**2 + reference_stds**2 + EPSILON_VARIANCE)
    z_scores = np.abs(diff) / pooled_std
    
    agreement_probs = np.exp(-(z_scores**2) / (2 * agreement_scale**2))
    ...
    # BAI score (posterior mean)
    bai_score = alpha_post / (alpha_post + beta_post)
    
    # EXACT credible intervals using Beta distribution
    alpha_level = (1 - credible_level) / 2
    lower_credible = beta_dist.ppf(alpha_level, alpha_post, beta_post)
    upper_credible = beta_dist.ppf(1 - alpha_level, alpha_post, beta_post)
    
    mean_agreement = np.mean(agreement_probs)
    
    return (float(bai_score), float(lower_credible), float(upper_credible), float(mean_agreement))



class devLINCSGCTXReader:
    """
